...
from .project import ProjectWorkspaceRuntime
...
class GPTResearcher:
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.project_title = kwargs.pop('project_title', None)
        self.project_capability = kwargs.pop('project_capability', 'deep_research')
        self.project_profile = kwargs.pop('project_profile', 'balanced')
        self.project_workspace = self._init_project_workspace()
        self.project_workspace_summary: dict[str, Any] | None = None
    ...
    def _init_project_workspace(self) -> Optional[ProjectWorkspaceRuntime]:
        if not any([self.project_root, self.project_id, self.project_title]):
            return None
        root = self.project_root or os.getenv("GPT_RESEARCHER_PROJECT_ROOT") or "./research_projects"
        title = self.project_title or self.query
        runtime = ProjectWorkspaceRuntime(
            root=root,
            title=title,
            project_id=self.project_id,
            capability=self.project_capability,
            profile=self.project_profile,
        )
        runtime.ensure_seed_topic(self.query)
        runtime.add_checkpoint(
            "research_scope_review",
            f"Review scope and next steps for query: {self.query}",
            options=["continue", "refine scope", "add constraints"],
        )
        return runtime
    ...
    async def write_report(
        ...
        if self.project_workspace:
            self.project_workspace_summary = self.project_workspace.sync_from_research(
                report=report,
                research_sources=self.research_sources,
                context=self.context,
                visited_urls=self.get_source_urls(),
            )
        return report
    ...
    def get_project_workspace_summary(self) -> dict[str, Any] | None:
        """Get the latest synchronized project workspace summary, if project mode is enabled."""
        return self.project_workspace_summary
